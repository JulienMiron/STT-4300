...
:::{.env-exemple titre="Jeu de données `turbines`"}
Les données `turbines`, tirées de la librairie `GLMsData` [@dunn2018glm], proviennent d'une expérience de fiabilité sur des roues de turbine (Nelson, 1982) : on y rapporte, pour différentes durées de fonctionnement, le nombre de roues ayant développé des fissures parmi celles mises à l'essai.
...
:::{.env-exemple titre="Introduction des termes quadratiques et cubiques dans l'exemple `turbines`"}
...
:::{.env-exemple titre="Sélection automatique sur le jeu de données `nminer`"}
...
::: {.env-exemple titre="Test d'adéquation avec l'exemple `turbines`"}
```{r}
library(GLMsData)
data(turbines)
