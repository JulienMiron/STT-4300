...
Dans ce qui suit, $\widehat{\text{V}}(\hat{\boldsymbol\beta})$ sera utilisé à la place de $\widehat{\text{Var}}(\hat{\boldsymbol\beta})$, pour alléger la notation. De plus, on écrit $\widehat{\text{V}}(\hat{\beta}_j)=[\widehat{\text{Var}}(\hat{\boldsymbol\beta})]_{jj}$ quand on parle de la variance estimée du paramètre $\beta_j$. 

**Forme explicite dans le cas binomial.**
On montre que

$$
\widehat{\text{V}}(\hat{\boldsymbol\beta})  = (X^\top W X)^{-1},
$$
...
$$\left[\hat\beta_j - z_{\alpha/2}\sqrt{\hat{V}(\hat\beta_j)}\,;\ \hat\beta_j + z_{\alpha/2}\sqrt{\hat{V}(\hat\beta_j)}\right],$$
où $z_\gamma$ est le quantile de la $\mathcal{N}(0,1)$ défini par $P(Z > z_\gamma) = \gamma$.

Pour tester $H_0: \beta_j = 0$ contre $H_1: \beta_j\neq 0$, on rejette $H_0$ au seuil $\alpha$ si $|z_{obs}| > z_{\alpha/2}$, avec $z_{obs} = \hat\beta_j/\sqrt{\hat{V}(\hat\beta_j)}$. Le seuil observé ($p$-valeur) est $\alpha^* = 2P(Z > |z_{obs}|)$.
