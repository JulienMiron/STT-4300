...
- **Logit :** $g(u) = \mathrm{logit}(u) = \log\!\left(\dfrac{u}{1-u}\right)$, d'où $\pi_i = \dfrac{e^{\eta_i}}{1+e^{\eta_i}}$. En R : `link="logit"` dans `glm`. Il s'agit du lien le plus utilisé en régression binaire. Ceci est en partie expliqué par son interprétabilité, voir la remarque [cette remarque](#sec-cotes).
...
$$
\hat{\boldsymbol\beta} \;\approx\; \mathcal{N}\{\boldsymbol\beta,\, v(\hat{\boldsymbol\beta})\}.
$$
...
n <- 20  # taille de chaque groupe

resultats <- map_dfr(1:(n - 1), function(k) {
  y1 <- c(rep(1, k), rep(0, n - k))
  y0 <- c(rep(1, n / 2), rep(0, n / 2))
  ...
  wald_z2  <- (beta_hat / et_hat)^2
  lr_stat  <- as.numeric(2 * (logLik(mod_complet) - logLik(mod_reduit)))

  data.frame(k = k, beta_hat = beta_hat, wald_chisq = wald_z2, lr_chisq = lr_stat)
})
...
resultats |>
  ...
  labs(x = expression(hat(beta)[x]), y = "Statistique de test (échelle khi-carré, 1 ddl)",
       color = NULL,
       title = "Effet Hauck-Donner : Wald redescend, le RV continue de croître") +
  theme_minimal()
```
...
#| warning: false
ajuster_exemple <- function(k, n = 20) {
  y1 <- c(rep(1, k), rep(0, n - k))
  y0 <- c(rep(1, n / 2), rep(0, n / 2))
  data.frame(y = c(y1, y0), x = rep(c(1, 0), each = n))
}

# Cas 1 : k = 15, pas de séparation
dat_normal <- ajuster_exemple(15)
mod_normal <- glm(y ~ x, data = dat_normal, family = binomial)
summary(mod_normal)$coefficients

# Cas 2 : k = 19, séparation quasi-complète
dat_separe <- ajuster_exemple(19)
mod_separe <- glm(y ~ x, data = dat_separe, family = binomial)
summary(mod_separe)$coefficients
```

Et le test du rapport de vraisemblance correspondant pour le cas séparé, à comparer à la p-value de Wald ci-dessus :
...
1. **Règle du delta** : $v(\hat\pi^*) = h({X^*}^\top\hat\beta)\, v({X^*}^\top\hat\beta)$ avec $h(u) = \{\partial g^{-1}(u)/\partial u\}^2$, d'où l'IC approximatif $\left[\hat\pi^* \pm z_{\alpha/2}\sqrt{v(\hat\pi^*)}\right]$.
...
summary(modele.logit)
```

Les trois liens mènent à des coefficients significatifs et à des courbes prédites $\hat\pi^*$ très similaires. Les intervalles de confiance se calculent avec `predict(..., se.fit=TRUE)` sur l'échelle du lien (méthode 1) ou avec `type="response"` (méthode 2).
:::
