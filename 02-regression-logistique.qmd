...
```{webr-r courbe-prob-turbines}
#| warning: false
library(ggplot2)
library(dplyr)

# Grille de valeurs de Hours pour une courbe lisse
grille <- data.frame(
  Hours = seq(min(turbines$Hours), max(turbines$Hours), length.out = 200)
)

# Fonction utilitaire pour extraire les prédictions d'un modèle donné
...
# Proportions observées, pour superposer les points aux données réelles
