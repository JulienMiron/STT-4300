...
mod1 <- glm(Detecte ~  Area + Grazed + Shrubs  + Timber,
            data = nminer, family = binomial)

mod2 <- glm(Detecte ~ Eucs + Grazed + Bulokes,
            data = nminer, family = binomial)
...
mod <- glm(cbind(survive, total - survive) ~ location + period,
...
```{r}
mod_quasi <- glm(cbind(survive, total - survive) ~ location + period,
...
pi_tilde <- troutegg$survive / troutegg$total
