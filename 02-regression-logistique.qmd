...
```
Le graphique des résidus vs valeurs ajustées montre plusieurs points nettement à l'écart (dont les observations 15 et 20), suggérant que le modèle ne s'ajuste pas également bien à toutes les combinaisons de `location` et `period`.

```{r}
plot(mod_quasi, which = 2)
```

Le QQ-plot des résidus s'éloigne fortement de la droite de référence dans la queue supérieure (observations 14, 15 et 20), ce qui traduit un mauvais ajustement pour certaines observations plutôt qu'une déviation normale isolée.

```{r}
plot(mod_quasi, which = 3)
```

Le graphique Scale-Location révèle une dispersion des résidus qui n'est pas constante et plusieurs valeurs extrêmes (1, 14, 15, 20), renforçant l'hypothèse d'une surdispersion mal capturée par le modèle binomial standard.

```{r}
plot(mod_quasi, which = 4)
```

La distance de Cook pointe vers certaines observations influentes (14, 19 et 20).
