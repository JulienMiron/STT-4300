...
::: {.env-exemple titre="Adéquation sur `danishlc`"}
```{r deviance}
mu_hat <- fitted(modele.additif)
n <- nrow(danishlc)
p <- length(coef(modele.additif)) - 1

X2 <- sum((danishlc$Cases - mu_hat)^2 / mu_hat)
...
:::{.env-remarque titre="Adéquation et taille des $\mu_i$"}
...
:::{.env-exemple titre="Effet de la ville sur [`danishlc`](#data-danishlc)"}
