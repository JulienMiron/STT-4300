...
:::{.env-exemple titre="Jeu de données [`empress`]{#data-empress}"}
...
1. `empress_agrege.csv` : une ligne par strate classe $\times$ sexe $\times$ groupe d'âge (11 strates, aucun garçon ne voyageant en première classe), avec le nombre de passagers $m_i$ (`effectif`) et le nombre de survivants $y_i$ (`survivants`).

Cette section utilise la version agrégée ; la version individuelle est utilisée au chapitre sur la validation croisée.

```{r empress-chargement}
#| message: false
library(dplyr)
library(ggplot2)
...
:::{.env-exemple titre="Ajustement sur [`empress`](#data-empress)"}
...
::: {.env-exemple titre="Adéquation sur [`empress`](#data-empress)"}
```{r empress-deviance}
...
:::{.env-exemple titre="Test de surdispersion sur [`empress`](#data-empress)"}
...
:::{.env-exemple titre="Effet de la classe sur [`empress`](#data-empress)"}
...
:::{.env-exemple titre="Diagnostics sur [`empress`](#data-empress)"}
