...
\makeatother
\hypersetup{
  pdftitle={STT-4300 - Régression II},
...
de l'Université Laval et Eva Cantoni de l'Université de Genève.
...
observations. L'estimateur sans biais usuel de \(\sigma^2\) est
...
l'estimation de \(\sigma\). L'inspection graphique des résidus permet de
vérifier :
...
; une valeur dépassant \(2(p+1)/n\) mérite examen. La distance de Cook,
\[D_i = \frac{r_i^2}{p+1}\cdot\frac{h_i}{1-h_i},\] mesure l'influence
globale de l'observation \(i\) sur les coefficients estimés. Les
\textbf{DFBETAS} et \textbf{DFFITS} mesurent respectivement l'effet du
retrait de l'observation \(i\) sur chaque coefficient et sur la valeur
ajustée \(\hat Y_i\).
...
Quatre exemples tirés de la librairie \texttt{GLMsData} de R :

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \texttt{quilpie} : observations de 1921 à 1988 de la quantité de pluie
  mesurée en juillet à Quilpie (Australie). La réponse vaut 1 si la
  quantité de pluie dépasse 10 mm et 0 sinon ; les variables
  explicatives concernent la pression atmosphérique (SOI).
\item
  \texttt{nminer} : 31 observations ; la réponse vaut 1 si un mineur
  bruyant (oiseau) est détecté dans la parcelle et 0 sinon.
\item
  \texttt{deposit} : 18 observations ; la réponse \(Y_i\) est le nombre
  d'insectes tués parmi les \(m_i\) insectes exposés, avec deux
  variables explicatives (type d'insecticide \(A\), \(B\) ou \(C\), et
  dose).
\item
  \texttt{turbines} : 11 observations ; la réponse \(Y_i\) est le nombre
  de machines présentant des fissures parmi \(m_i\) machines, en
...
\pandocbounded{\includegraphics[keepaspectratio]{02-regression-logistique_files/figure-pdf/unnamed-chunk-1-1.pdf}}
...
\textbf{Les cotes (odds).} Le rapport ci-dessus a une interprétation
concrète : c'est la cote (\emph{odds}), notée \(o_i\) :
...
constantes, \(\eta_i\) augmente de \(\beta_j\). Comme
\(o_i = e^{\eta_i}\), cette augmentation se traduit par une
...
{Exemple --- Jeu de données \texttt{turbines}}

Les données \texttt{turbines}, tirées de la librairie \texttt{GLMsData}
(Dunn et Smyth 2018), proviennent d'une expérience de fiabilité sur des
roues de turbine (Nelson, 1982) : on y rapporte, pour différentes durées
de fonctionnement, le nombre de roues ayant développé des fissures parmi
celles mises à l'essai.
...
\pandocbounded{\includegraphics[keepaspectratio]{02-regression-logistique_files/figure-pdf/unnamed-chunk-2-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-regression-logistique_files/figure-pdf/unnamed-chunk-3-1.pdf}}
...
grand (par exemple en présence de séparation quasi-complète ou d'un
effet très fort), la courbure de la vraisemblance en \(\hat\beta_j\)
s'aplatit, ce qui \textbf{gonfle} \(\sqrt{\hat{\text V}(\hat\beta_j)}\)
et peut faire \textbf{diminuer} \(z_{obs}\) --- donnant l'impression que
l'effet n'est pas significatif, alors qu'il l'est. On pourrait montrer
que cela est causé par le fait que l'erreur-type augmente plus
...
\pandocbounded{\includegraphics[keepaspectratio]{02-regression-logistique_files/figure-pdf/hauck-donner-sim-continu-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-regression-logistique_files/figure-pdf/hauck-donner-exemples-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-regression-logistique_files/figure-pdf/hauck-donner-exemples-2.pdf}}
...
{Remarque --- Règle du delta avec le lien logit}
...
\(\hat\theta \approx \mathcal{N}(\theta,\, \hat{\text V}(\hat\theta))\).
...
\textbf{Le modèle saturé.} Pour juger si un modèle avec \(p+1\)
paramètres \(\boldsymbol\beta\) s'ajuste bien aux données, il est utile
de le comparer au modèle le plus flexible possible : celui qui accorde à
chaque observation son propre paramètre, sans aucune structure imposée
par des variables explicatives. On l'appelle le \textbf{modèle saturé},
avec \(n\) paramètres \(\{\tilde\mu_1,\ldots,\tilde\mu_n\}\) (un par
observation, plutôt que \(p+1\) pour l'ensemble de l'échantillon).
...
::: \{.env-definition titre=``test''\} fff :::

On définit la déviance comme étant deux fois le log de ce rapport, ce
qui arrive à
...
par le modèle. Puisque le modèle saturé maximise la vraisemblance parmi
\emph{tous} les modèles possibles, \(l(y,y) \geq l(\hat\mu,y)\)
toujours, et donc \(D \geq 0\) : plus \(D\) est petit, plus le modèle
considéré se rapproche du modèle saturé, donc plus il s'ajuste bien aux
données. La déviance joue le même rôle que la somme des carrés des
résidus en régression linéaire : elle sert à évaluer l'adéquation du
modèle et à comparer des modèles.
...
\textbf{Exemple (\texttt{turbines}, termes quadratique et cubique).}
...
(fonction \texttt{step} avec l'argument \texttt{direction}). Sur le jeu
de données \texttt{deposit}, les trois procédures retiennent le modèle
incluant le type d'insecticide, la dose et le terme quadratique de la
dose.
...
déviance \(D = \sum_i r_{D,i}^2\) et la statistique du khi-deux de
Pearson
...
Ces tests ne sont valides que pour des \textbf{données groupées} avec
des \(m_i\) élevés, car ils reposent sur un résultat asymptotique en
\(m_i\). Ils sont inutilisables pour des données individuelles
(Bernoulli).
...
valeur absolue, cela peut suggérer une \textbf{extra-variabilité}
(\emph{overdispersion}) : \(E[Y_i] = m_i\pi_i\) mais
\(Var[Y_i] > m_i\pi_i(1-\pi_i)\). On pose alors
\(Var[Y_i] = \phi\, m_i\pi_i(1-\pi_i)\) et on estime \(\phi\) par
\(\chi^2_P/(n-(p+1))\). En cas d'extra-variabilité, on spécifie
\texttt{family=quasibinomial()} au lieu de \texttt{family=binomial()}
dans \texttt{glm}.
...
\(\hat y_i = 1\) si \(\hpi{i} > u\) et \(\hat y_i = 0\) sinon. On mesure
la capacité prédictive par :
...
\subsection{Déviance}\label{duxe9viance-1}
...
\pandocbounded{\includegraphics[keepaspectratio]{08-gam_files/figure-pdf/unnamed-chunk-1-1.pdf}}

\pandocbounded{\includegraphics[keepaspectratio]{08-gam_files/figure-pdf/unnamed-chunk-1-2.pdf}}
